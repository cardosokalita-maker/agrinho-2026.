...
// 5. Redirecionamento Dinâmico para a Página de Detalhes
const cardsTecnologia = document.querySelectorAll("#tecnologias .card3d");

cardsTecnologia.forEach(card => {
    card.addEventListener("click", () => {
        const techTipo = card.getAttribute("data-tech");
        if (techTipo) {
            window.location.href = `tecnologias.html?tipo=${techTipo}`;
        }
    });
});